...
def Manchester_RLL_encode(data):
  result = ''
  for i in data:
    if i == '1':
      result += '10'
...
#10 
#01
def Manchester_RLL_decode(data):
  result = ''
  for i in range(int(len(data) / 2)):
    if data[i * 2:i * 2+2] == '10':
      result += '1'
    # guess
    elif data[i * 2:i * 2+2] == '11':
      result += '1'
    elif data[i * 2:i * 2+2] == '01':
      result += '0'
    else:
      result += '0'
  return result
...
def read_light_rll_rs_conv(channel, threshold):
  global signal
  flag = False
  adc = ADC()
  value = 0
  ...
  while True:
    if signal == 0:
      ...
      if flag:
        #conv decode
        rll_code = Manchester_RLL_decode(m)
        d_code, corrected_errors = conv_code.decoder(rll_code)
...
def rll_rs_conv_encode(msg, rsc):
    #msg_b len(msg) B
    ...
    code = Manchester_RLL_encode(code)
    #random choose 5% index to flip
    random_numbers = random.sample(number_range, int(len(code) * 0.05))
    code_ = bitarray(code)
    for i in random_numbers:
        code_[i] = not (code_[i])
    code_ = str(code_)[10:-2]
    print('length of RLL code',end = ': ')
    print(len(code_) / 8)
    print()
    return code_
...
pin = sh.argv2pin()
#test conv code
thread1 = threading.Thread(target=read_light_rll_rs_conv,args = [pin,400,]).start()
rsc = RSCodec(10) #can tolerate 5 error
code_ = rll_rs_conv_encode('hello word', rsc)
send_msg(code_)
...
code_ = rll_rs_conv_encode('ITCT final project', rsc)
send_msg(code_)
